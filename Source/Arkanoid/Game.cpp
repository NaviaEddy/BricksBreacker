...
void AGame::SetLevel(AActor* Level)
{
	...
	if (!LevelBuilder) { //Lanzamos un error si la conversion falla
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, TEXT("�Elenco no v�lido! Consulte Registro de salida para obtener m�s detalles."));
		UE_LOG(LogTemp, Error, TEXT("SetLevel(): �El actor no es un LevelBuilder! �Est� seguro de que el actor implementa esa interfaz?"));
	}
}
...
void AGame::ConstructOfLevels(int _Level)
{
	...
	if (!LevelBuilder) {
		UE_LOG(LogTemp, Error, TEXT("ConstructLevel1(): LevelBuilder es NULL, aseg�rese de que este inicializado."));
		return;
	}
	else {
